...
| ene_ana | `ene_ana @en_files aladip_solvated/expected/energies.tre @library .local/gromosXX/md++/data/ene_ana.md++.lib @prop …` — `plain` (`totene totpot totkin`), `usertime` (`@prop totene @time 5 0.5`), `topo_densit` (`@prop densit totene @topo shared/aladip.topo`), `direct_refs` (`@prop 'ENER[1]' 'ENER[11]'`), `two_files` (the same `.tre` twice). stdout and every `<prop>.dat` compared. The library is md++'s own, **not** the LiveCoMS tutorial's copy: both claim `ENEVERSION 2023-04-15` but the tutorial's declares `ENER 50`/`SPECIAL 12` against md++'s `ENER 52`/`SPECIAL 13`, so it does not describe a current `.tre` — the tutorial text itself says to take the library from `md++-x.y.z/data`. |
