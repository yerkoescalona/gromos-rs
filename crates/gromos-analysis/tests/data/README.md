...
| rmsd | `rmsd @topo shared/aladip.topo @pbc r @atomsrmsd 1:a @traj aladip.trc` (`fit_all`), `… @atomsrmsd 1:3-8 @atomsfit 1:a` (`fit_separate`), `… @atomsrmsd 1:a @ref aladip_solvated/expected/final.conf @time 5 0.5` (`ref_conf`) |
| rmsf | `rmsf @topo shared/aladip.topo @pbc r @atomsrmsf 1:a @traj aladip.trc` (`fit_all`), `… @atomsrmsf 1:3-8 @atomsfit 1:a @ref aladip_solvated/expected/final.conf` (`fit_separate`) |
| ene_ana | `ene_ana @en_files aladip_solvated/expected/energies.tre @prop …` (gromos++ run with `@library .local/gromosXX/md++/data/ene_ana.md++.lib`; ours with the built-in library, generated from the same layout — `crates/gromos-io/data/ene_ana.md++.lib`) — `plain` (`totene totpot totkin`), `usertime` (`@prop totene @time 5 0.5`), `topo_densit` (`@prop densit totene @topo shared/aladip.topo`), `direct_refs` (`@prop 'ENER[1]' 'ENER[11]'`), `two_files` (the same `.tre` twice). stdout and every `<prop>.dat` compared. The library is md++'s own, **not** the LiveCoMS tutorial's copy: both claim `ENEVERSION 2023-04-15` but the tutorial's declares `ENER 50`/`SPECIAL 12` against md++'s `ENER 52`/`SPECIAL 13`, so it does not describe a current `.tre` — the tutorial text itself says to take the library from `md++-x.y.z/data`. Our `ene_ana` refuses the tutorial's copy at open (layout fingerprint mismatch, `docs/src/reference/energy-library.md`). |
