...
fn main() {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--winframe" => {
                i += 1;
                winframe = args[i].parse().unwrap_or(10);
            },
            "--pbc" => {
                do_pbc = true;
            },
    ...
    let atom_spec = atom_spec.unwrap_or_else(|| {
        eprintln!("Error: @atoms required");
        process::exit(1);
    });
    ...
    let n_sel = AtomSelection::from_string(&atom_spec, &topo).unwrap_or_else(|e| {
        eprintln!("Error in @atoms '{atom_spec}': {e}");
        process::exit(1);
    });
    ...
    let fit_sel = AtomSelection::from_string(fit_spec_str, &topo).unwrap_or_else(|e| {
        eprintln!("Error in @atomsfit '{fit_spec_str}': {e}");
        process::exit(1);
    });
