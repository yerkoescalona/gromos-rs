    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "@fitstart" => {
                i += 1;
                if i >= args.len() {
                    return Err("Missing value for @fitstart".to_string());
                }
                diffus_args.fit_start = Some(
                    args[i]
                        .parse()
            ...
            "@fitend" => {
                i += 1;
                if i >= args.len() {
                    return Err("Missing value for @fitend".to_string());
                }
                diffus_args.fit_end = Some(
                    args[i]
                        .parse()
...
fn calculate_msd(
    ...
    for (frame_idx, frame) in all_frames.iter().enumerate() {
        ...
        let positions: Vec<Vec3> = atoms.iter().map(|&idx| frame.positions[idx]).collect();
...
fn main() {
    ...
    let filter = if diffus_args.verbose > 0 {
        "debug"
    } else {
        "info"
    };
