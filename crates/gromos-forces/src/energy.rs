...
use gromos_core::{
    configuration::{Box as SimBox, BoxType, Configuration, Energy},
...
use crate::{
    ...
    nonbonded::{
        CRFParameters, ForceStorage, LJParamMatrix, LJParameters,
        lj_crf_innerloop_novirial, rf_excluded_interactions, one_four_interaction_loop,
...
impl SinglePointEnergy {
    fn update_potential(&mut self) {
        self.potential = self.bond + self.angle + self.dihedral + self.improper
            + self.lj + self.crf + self.special;
    }
}
...
pub fn single_point_energy(
    ...
    let lj_nested: Vec<Vec<LJParameters>> = topo.lj_parameters
        .iter().map(|r| r.iter().map(LJParameters::from).collect()).collect();
    let lj_mat = LJParamMatrix::from_nested(&lj_nested);
    let crf      = CRFParameters::new(params.cutoff, params.epsilon, params.rf_epsilon, params.rf_kappa);

    let pairlist_u32: Vec<(u32,u32)> = pairlist.solute_short.iter()
        .chain(pairlist.solute_long.iter())
        .map(|&(i,j)| (i as u32, j as u32)).collect();
    let solvent_u32: Vec<(u32,u32)> = pairlist.solvent_short.iter()
        .chain(pairlist.solvent_long.iter())
        .map(|&(i,j)| (i as u32, j as u32)).collect();

    // ── Bonded ──────────────────────────────────────────────────────────────
    let bonded = calculate_bonded_forces_ntf(
        topo, &conf, params.quartic_bonds,
        params.ntf[0], params.ntf[1], params.ntf[3], params.ntf[2],
    );
    ...
    if !pairlist_u32.is_empty() {
        lj_crf_innerloop_novirial(
            &conf.current().pos, &charges, &iac_u32,
            &pairlist_u32, &lj_mat, &crf, &periodicity, &mut nb,
        );
    }
    if !solvent_u32.is_empty() {
        solvent_innerloop_novirial(
            &conf.current().pos, &charges, &iac_u32,
            &solvent_u32, &lj_mat, &crf, &periodicity,
            params.atoms_per_solvent, &mut nb,
        );
    }

    rf_excluded_interactions(
        &charges, &topo.exclusions, &conf.current().pos,
        &crf, &periodicity, &mut nb, topo.num_solute_atoms(),
    );

    if !topo.one_four_pairs.is_empty() {
        one_four_interaction_loop(
            &topo.one_four_pairs, &conf.current().pos,
            &charges, &iac_u32, &lj_mat, &crf, &periodicity, &mut nb, 1.0,
        );
    }
