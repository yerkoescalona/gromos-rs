...
impl ReactionFieldParameters {
    ...
    pub fn new(cutoff: f64, epsilon: f64, epsilon_rf: f64, kappa: f64) -> Self {
        ...
        let crf = if epsilon_rf == 0.0 {
            ...
            let numerator =
                2.0 * (epsilon - epsilon_rf) * (1.0 + kappa_cutoff) - epsilon_rf * kappa_cutoff_sq;

            let denominator =
                (epsilon + 2.0 * epsilon_rf) * (1.0 + kappa_cutoff) + epsilon_rf * kappa_cutoff_sq;
    ...
    match order {
        ...
        5 => {
            ...
            if u < 0.5 {
                ...
                let t = u - 0.5;
                let t2 = t * t;
                19.0 / 96.0 + 1.0 / 4.0 * t + 1.0 / 4.0 * t2
                    - 1.0 / 6.0 * t2 * t
                    - 1.0 / 6.0 * t2 * t2
            } else if u < 2.5 {
                let t = 2.5 - u;
        ...
        _ => {
            eprintln!(
                "Warning: B-spline order {} not implemented, using cubic",
                order
            );
            b_spline(4, x)
        },
...
fn fft_3d_inverse(
    ...
    let mut grid: Vec<Vec<Vec<Complex<f64>>>> = complex_grid
        ...
        .map(|plane| {
            plane
                .into_iter()
                .map(|row| row.into_iter().map(|c| Complex::new(c.re, c.im)).collect())
                .collect()
        })
        .collect();
...
pub fn pme_reciprocal_space(
    ...
    for i_x in 0..pme.grid_x {
        ...
        for i_y in 0..pme.grid_y {
            ...
            for i_z in 0..nz_half {
                ...
                let g_k =
                    pme_influence_function(k_x, k_y, k_z, pme.alpha, volume, pme.spline_order);
    ...
    let force_potential_grid = fft_3d_inverse(fourier_grid, pme.grid_x, pme.grid_y, pme.grid_z);
...
fn b_spline_derivative(order: usize, x: f64) -> f64 {
    ...
    match order {
        ...
        _ => {
            eprintln!(
                "Warning: B-spline derivative order {} not implemented",
                order
            );
            0.0
        },
