...
use gromos_core::topology::{AngleParameters, BondParameters, Topology};

// FEP support - we'll define a minimal LambdaController here for now
// In a full implementation, this would be in gromos_integrators

/// Per-interaction type lambda values for FEP
...
/// Minimal lambda controller for free energy calculations
#[derive(Debug, Clone, Default)]
...
impl LambdaController {
    ...
    pub fn with_lambda(mut self, lambda: f64) -> Self {
        self.lambda = lambda;
        // Set all interaction lambdas to the same value (can be customized later)
...
mod tests {
    ...
    use gromos_core::math::Vec3;
    use gromos_core::topology::{Atom, Topology};
    ...
    fn test_dihedral_new_at_minimum() {
        use gromos_core::configuration::Configuration;
        use gromos_core::math::Vec3;
        use gromos_core::topology::{Atom, Dihedral, DihedralParameters};
    ...
    fn test_crossdihedral_simple() {
        ...
        assert!(
            result.energy >= 0.0 && result.energy <= 20.0,
    ...
    fn test_crossdihedral_coupling() {
        ...
        assert!(
            ...
            "Energy for φ+ψ≈2π should be near 2K=20: got {}",
            result.energy
        );
    }
    ...
    fn test_perturbed_bond_lambda_interpolation() {
        use crate::bonded::LambdaController;
        use gromos_core::topology::{Atom, BondParameters, PerturbedBond};
        ...
        println!(
            "\nλ=0: Energy = {:.6}, Expected = {:.6}",
            result_0.energy, expected_energy_0
        );
        assert!(
            (result_0.energy - expected_energy_0).abs() < 1e-6,
        ...
        println!(
            "λ=1: Energy = {:.6}, Expected = {:.6}",
            result_1.energy, expected_energy_1
        );
        assert!(
            (result_1.energy - expected_energy_1).abs() < 1e-6,
        ...
        println!(
            "λ=0.5: Energy = {:.6}, Expected = {:.6}",
            result_05.energy, expected_energy_05
        );
        assert!(
            (result_05.energy - expected_energy_05).abs() < 1e-6,
        ...
        let expected_de_dlambda =
            0.5 * delta_k * diff_interp * diff_interp - k_interp * diff_interp * delta_r0;
        println!(
            "λ=0.5: dE/dλ = {:.6}, Expected = {:.6}",
            result_05.lambda_derivative, expected_de_dlambda
        );
        assert!(
            (result_05.lambda_derivative - expected_de_dlambda).abs() < 1e-6,
        ...
        println!(
            "Force conservation: |F_total| = {:.6e}",
            total_force.length()
        );
        assert!(
            total_force.length() < 1e-5,
    ...
    fn test_perturbed_angle_forces() {
        use crate::bonded::LambdaController;
        use gromos_core::topology::{Angle, AngleParameters, PerturbedAngle};
        ...
        assert!(
            (result_0.energy - expected_energy_0).abs() < 0.01,
            "Energy at λ=0 should match state A"
        );
        ...
        assert!(
            (result_1.energy - expected_energy_1).abs() < 0.01,
            "Energy at λ=1 should match state B"
        );
        ...
        assert!(
            (result_05.energy - expected_energy_05).abs() < 0.01,
            "Energy at λ=0.5 should be interpolated"
        );
        ...
        let expected_de_dlambda =
            0.5 * delta_k * diff_interp * diff_interp - k_interp * diff_interp * delta_theta0;
        println!(
            "λ=0.5: dE/dλ = {:.6}, Expected = {:.6}",
            result_05.lambda_derivative, expected_de_dlambda
        );
        assert!(
            (result_05.lambda_derivative - expected_de_dlambda).abs() < 0.01,
            "Lambda derivative should match analytical formula"
        );
        ...
        assert!(total_force.length() < 1e-4, "Forces should be conserved");
    ...
    fn test_perturbed_dihedral_forces() {
        use crate::bonded::LambdaController;
        use gromos_core::topology::{DihedralParameters, PerturbedDihedral};
        use std::f64::consts::PI;
        ...
        assert!(
            (result_0.energy - expected_energy_0).abs() < 0.1,
            "Energy at λ=0 should match state A"
        );
        ...
        assert!(
            (result_1.energy - expected_energy_1).abs() < 0.1,
            "Energy at λ=1 should match state B"
        );
        ...
        assert!(
            (result_05.energy - expected_energy_05).abs() < 0.1,
            "Energy at λ=0.5 should be interpolated"
        );
        ...
        let expected_de_dlambda =
            delta_k * (1.0 + arg_interp.cos()) - k_interp * arg_interp.sin() * delta_pd;
        println!(
            "λ=0.5: dE/dλ = {:.6}, Expected = {:.6}",
            result_05.lambda_derivative, expected_de_dlambda
        );
        ...
        // Verify force conservation
        let total_force =
            result_05.forces[0] + result_05.forces[1] + result_05.forces[2] + result_05.forces[3];
        println!(
            "Force conservation: |F_total| = {:.6e}",
            total_force.length()
        );
    ...
    fn test_perturbed_angle_lambda_scan() {
        ...
        for i in 0..n_lambda {
            ...
            println!(
                "{:.2}\t{:.6}\t\t{:.6}",
                lambda, result.energy, result.lambda_derivative
            );
        }
