...
//!
//! ```rust,ignore
//! use gromos_io::trajectory_binary::{DcdWriter, BinaryTrajectoryWriter};
...
///
/// ```rust,ignore
/// use gromos_io::trajectory_binary::DcdWriter;
///
...
impl DcdWriter {
    ...
    fn write_header(
        ...
        // Save frequency
        self.writer
            .write_i32::<LittleEndian>(self.save_frequency as i32)?;
        ...
        // Unused fields (8 ints) - includes CHARMM version placeholder in last position
        for _ in 0..8 {
            self.writer.write_i32::<LittleEndian>(0)?;
        }
    ...
    fn write_dcd_frame(&mut self, config: &Configuration) -> Result<(), IoError> {
        ...
        for pos in &state.pos {
            ...
            self.writer
                .write_f32::<LittleEndian>((pos.x * 10.0) as f32)?;
        }
        self.writer.write_i32::<LittleEndian>(block_size)?;
        ...
        for pos in &state.pos {
            self.writer
                .write_f32::<LittleEndian>((pos.y * 10.0) as f32)?;
        }
        self.writer.write_i32::<LittleEndian>(block_size)?;
        ...
        for pos in &state.pos {
            self.writer
                .write_f32::<LittleEndian>((pos.z * 10.0) as f32)?;
        }
        self.writer.write_i32::<LittleEndian>(block_size)?;
...
impl DcdReader {
    ...
    fn read_header(reader: &mut BufReader<File>) -> Result<(usize, usize, f32, u64), IoError> {
        ...
        if end_marker != 84 {
            return Err(IoError::FormatError(
                "Invalid DCD header end marker".to_string(),
            ));
        }
...
impl BinaryTrajectoryReader for DcdReader {
    fn read_frame(&mut self) -> Result<Option<BinaryFrame>, IoError> {
        ...
        if block_size != 48 {
            return Err(IoError::FormatError(
                "Invalid unit cell block size".to_string(),
            ));
        }
        ...
        if block_size != expected_size {
            return Err(IoError::FormatError(
                "Invalid X coordinate block size".to_string(),
            ));
        }
