use gromos_integrators::eds::EDSParameters;
...
impl EdsStatsWriter {
    ...
    pub fn write_frame(&mut self, step: usize, eds: &EDSParameters) -> Result<(), String> {
        ...
        writeln!(self.writer).map_err(|e| format!("Failed to write newline: {}", e))?;
...
impl EdsVrWriter {
    ...
    pub fn new(path: &str, title: &str) -> Result<Self, String> {
        ...
        writeln!(
            writer,
            "# Contains the reference energy V_R at each MD step"
        )
        .map_err(|e| format!("Failed to write header: {}", e))?;
        writeln!(
            writer,
            "# for computing ensemble averages and free energy differences."
        )
        .map_err(|e| format!("Failed to write header: {}", e))?;
        writeln!(writer, "#").map_err(|e| format!("Failed to write header: {}", e))?;
        writeln!(writer, "# Columns:").map_err(|e| format!("Failed to write header: {}", e))?;
    ...
    /// Write a V_R frame
    pub fn write_frame(&mut self, step: usize, time: f64, v_r: f64) -> Result<(), String> {
        writeln!(self.writer, "  {:8} {:12.6} {:12.6}", step, time, v_r)
...
mod tests {
    use super::*;
    use gromos_integrators::eds::{EDSForm, EDSState};
    use std::fs;
    ...
    fn test_eds_stats_writer() {
        ...
        let mut eds = EDSParameters {
            ...
            s_values: vec![0.5],
            temperature: 300.0,
            states: vec![EDSState::new(0, 0.0, 10), EDSState::new(1, 10.0, 10)],
