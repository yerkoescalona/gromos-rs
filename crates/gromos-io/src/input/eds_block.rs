...
/// END
/// ```
use gromos_integrators::eds::{AEDSParameters, EDSForm, EDSParameters};
use std::fs::File;
        ...
        for line in reader.lines() {
            ...
            if in_eds_block {
                ...
                if !parsed_header {
                    if parts.len() < 3 {
                        return Err(format!(
                            "Invalid EDS header at line {}: expected NUMSTATES FORM S",
                            line_num
                        ));
                    }

                    block.num_states = parts[0].parse::<usize>().map_err(|_| {
                        format!("Invalid NUMSTATES at line {}: {}", line_num, parts[0])
                    })?;

                    let form_val = parts[1]
                        .parse::<i32>()
                    ...
                    match block.form {
                        EDSForm::SingleS => {
                            if block.s_values.len() != 1 {
                                return Err(format!(
                                    "SingleS form requires exactly 1 s value, got {} at line {}",
                                    block.s_values.len(),
                                    line_num
                                ));
                            }
                ...
                // Line 2: E_OFFSETS (all on one line)
                    ...
                    if block.e_offsets.len() != block.num_states {
                        return Err(format!(
                            "Expected {} energy offsets for {} states, got {} at line {}",
                            block.num_states,
                            block.num_states,
                            block.e_offsets.len(),
                            line_num
                ...
                // Line 3: TEMP
                    ...
                    if parts.len() != 1 {
                        return Err(format!(
                            "Invalid TEMP line at {}: expected single value",
                            line_num
                        ));
                    }

                    block.temperature = parts[0]
                        .parse::<f64>()
                ...
                // Line 4 (optional): SEARCH  E_MAX  E_MIN
                    ...
                    let search = parts[0].parse::<i32>().map_err(|_| {
                        format!("Invalid SEARCH at line {}: {}", line_num, parts[0])
                    })?;
