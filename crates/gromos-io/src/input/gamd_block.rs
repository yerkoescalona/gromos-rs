...
/// END
/// ```
use gromos_integrators::gamd::{BoostForm, GamdParameters, SearchMode, ThresholdType};
use std::fs::File;
...
impl GamdBlock {
    ...
    pub fn parse(reader: &mut dyn BufRead) -> Result<Option<Self>, String> {
        ...
        let mut line_num = 0;
        let mut parsed_search_line = false;
        let mut parsed_sigma_line = false;
        ...
        for line in reader.lines() {
            ...
            if trimmed == "GAMD" {
                ...
                parsed_search_line = false;
                parsed_sigma_line = false;
                continue;
            }
            ...
            if in_gamd_block {
                ...
                if parts.len() == 3 && !parsed_search_line {
                    parsed_search_line = true;
                    let search = parts[0].parse::<i32>().map_err(|_| {
                        format!("Invalid SEARCH value at line {}: {}", line_num, parts[0])
                    })?;
                    let form = parts[1].parse::<i32>().map_err(|_| {
                        format!("Invalid FORM value at line {}: {}", line_num, parts[1])
                    })?;
                    let thresh = parts[2].parse::<i32>().map_err(|_| {
                        format!("Invalid THRESH value at line {}: {}", line_num, parts[2])
                    })?;
                ...
                else if parts.len() == 2 && !parsed_sigma_line && parsed_search_line {
                    parsed_sigma_line = true;
                    block.sigma0_dih = parts[0].parse::<f64>().map_err(|_| {
                        format!("Invalid SIGMA0_DIH at line {}: {}", line_num, parts[0])
                    })?;
                    block.sigma0_tot = parts[1].parse::<f64>().map_err(|_| {
                        format!("Invalid SIGMA0_TOT at line {}: {}", line_num, parts[1])
                    })?;
                }
                // Line 3 (optional): K_DIH  K_TOT  E_DIH  E_TOT
                else if parts.len() == 4 && block.k_dih.is_none() {
                    block.k_dih = Some(parts[0].parse::<f64>().map_err(|_| {
                        format!("Invalid K_DIH at line {}: {}", line_num, parts[0])
                    })?);
                    block.k_tot = Some(parts[1].parse::<f64>().map_err(|_| {
                        format!("Invalid K_TOT at line {}: {}", line_num, parts[1])
                    })?);
                    block.e_dih = Some(parts[2].parse::<f64>().map_err(|_| {
                        format!("Invalid E_DIH at line {}: {}", line_num, parts[2])
                    })?);
                    block.e_tot = Some(parts[3].parse::<f64>().map_err(|_| {
                        format!("Invalid E_TOT at line {}: {}", line_num, parts[3])
                    })?);
                }
                // Line 4 or 3: EQUIL  WINDOW (must come after sigma line is parsed)
                else if parts.len() == 2 && parsed_sigma_line {
                    block.equilibration_steps = parts[0]
                        .parse::<usize>()
                        .map_err(|_| format!("Invalid EQUIL at line {}: {}", line_num, parts[0]))?;
                    block.window_size = parts[1].parse::<usize>().map_err(|_| {
                        format!("Invalid WINDOW at line {}: {}", line_num, parts[1])
                    })?;
                }
            }
        }
