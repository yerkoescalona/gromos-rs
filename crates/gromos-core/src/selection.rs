...
impl AtomSelection {
    ...
    fn parse_atom_list(spec: &str, n_atoms: usize) -> Result<Self, SelectionError> {
        ...
        for part in spec.split(',') {
            ...
            if part.contains('-') {
                ...
                let start: usize = range[0].trim().parse().map_err(|_| {
                    SelectionError::ParseError(format!("Invalid start: {}", range[0]))
                })?;
                let end: usize = range[1].trim().parse().map_err(|_| {
                    SelectionError::ParseError(format!("Invalid end: {}", range[1]))
                })?;
                ...
                if atom < 1 {
                    return Err(SelectionError::InvalidRange(
                        "Atom numbers must be >= 1".to_string(),
                    ));
                }
    ...
    fn parse_prefixed(spec: &str, topology: &Topology) -> Result<Self, SelectionError> {
        ...
        match prefix {
            ...
            mol_str => {
                // Molecule number
                let mol: usize = mol_str.parse().map_err(|_| {
                    SelectionError::ParseError(format!("Invalid molecule: {}", mol_str))
                })?;
