...
pub fn validate_topology(topo: &Topology) -> ValidationReport {
    ...
    if n_atoms == 0 {
        report.add(ValidationError::fatal(
            "topology",
            "No atoms in topology".to_string(),
        ));
        return report;
    }
    ...
    for (i, bond) in topo.solute.bonds.iter().enumerate() {
        if bond.i >= n_atoms || bond.j >= n_atoms {
            report.add(ValidationError::error(
                "topology",
                format!(
                    "Bond {} references out-of-range atoms: {}-{}",
                    i,
                    bond.i + 1,
                    bond.j + 1
                ),
    ...
    for (i, angle) in topo.solute.angles.iter().enumerate() {
        if angle.i >= n_atoms || angle.j >= n_atoms || angle.k >= n_atoms {
            report.add(ValidationError::error(
                "topology",
                format!(
                    "Angle {} references out-of-range atoms: {}-{}-{}",
                    i,
                    angle.i + 1,
                    angle.j + 1,
                    angle.k + 1
    ...
    for (i, dih) in topo.solute.proper_dihedrals.iter().enumerate() {
        if dih.i >= n_atoms || dih.j >= n_atoms || dih.k >= n_atoms || dih.l >= n_atoms {
            report.add(ValidationError::error(
                "topology",
                format!(
                    "Dihedral {} references out-of-range atoms: {}-{}-{}-{}",
                    i,
                    dih.i + 1,
                    dih.j + 1,
                    dih.k + 1,
...
pub fn validate_configuration(topo: &Topology, conf: &Configuration) -> ValidationReport {
    ...
    if n_topo != n_conf {
        report.add(ValidationError::fatal(
            "configuration",
            format!(
                "Topology has {} atoms but coordinates have {}",
                n_topo, n_conf
            ),
        ));
        return report;
...
pub fn validate_energy(
    ...
    if !kinetic.is_finite() {
        report.add(ValidationError::error(
            "energy",
            "Kinetic energy is not finite".to_string(),
        ));
    }
    if !potential.is_finite() {
        report.add(ValidationError::error(
            "energy",
            "Potential energy is not finite".to_string(),
        ));
    }
    if !total.is_finite() {
        report.add(ValidationError::error(
            "energy",
            "Total energy is not finite".to_string(),
        ));
    }
    if !temperature.is_finite() {
        report.add(ValidationError::error(
            "energy",
            "Temperature is not finite".to_string(),
        ));
    }
    ...
    if energy_diff > 0.01 * total.abs() {
        report.add(ValidationError::warning(
            "energy",
            format!(
                "Energy sum mismatch: K+V = {:.4}, Total = {:.4}",
...
pub fn check_energy_drift(energies: &[(f64, f64, f64)]) -> ValidationReport {
    ...
    if relative_drift > 0.01 {
        ...
        report.add(ValidationError::warning(
            "energy_drift",
            format!(
                "Significant energy drift: {:.2}% ({:.4} kJ/mol over {} frames)",
    ...
    for i in 1..energies.len() {
        ...
        if relative_jump > 0.1 {
            // 10% jump between frames
            report.add(ValidationError::error(
                "energy_jump",
                format!(
                    "Large energy jump at frame {}: {:.2}% ({:.4} kJ/mol)",
                    i,
                    relative_jump * 100.0,
                    jump
                ),
