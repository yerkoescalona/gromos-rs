...
use gromos::selection::AtomSelection;
use std::env;
use std::process;
...
fn main() {
    ...
    eprintln!(
        "  Fit:        {}",
        if parsed_args.do_fit { "yes" } else { "no" }
    );
    eprintln!();
    ...
    let ref_frame_idx = match parsed_args.ref_spec.to_lowercase().as_str() {
        "first" => 0,
        "last" => frames.len() - 1,
        _ => match parsed_args.ref_spec.parse::<usize>() {
            Ok(idx) => {
                if idx >= frames.len() {
                    eprintln!(
                        "Error: Reference frame {} out of range (0-{})",
                        idx,
                        frames.len() - 1
                    );
                    process::exit(1);
            ...
            Err(_) => {
                eprintln!(
                    "Error: Invalid reference specification '{}'",
                    parsed_args.ref_spec
                );
    ...
    eprintln!(
        "  Reference frame: {} (time: {:.4} ps)",
        ref_frame_idx, frames[ref_frame_idx].time
    );
    eprintln!();
