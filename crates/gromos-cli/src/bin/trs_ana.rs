...
fn parse_args(args: Vec<String>) -> Result<TrajectoryArgs, String> {
    ...
    Ok(TrajectoryArgs { traj_file, verbose })
}
...
fn main() {
    ...
    println!(
        "  Mean:   ({:.4}, {:.4}, {:.4}) nm",
        coord_stats.mean.x, coord_stats.mean.y, coord_stats.mean.z
    );
    println!(
        "  Std:    ({:.4}, {:.4}, {:.4}) nm",
        coord_stats.std_dev.x, coord_stats.std_dev.y, coord_stats.std_dev.z
    );
    println!(
        "  Min:    ({:.4}, {:.4}, {:.4}) nm",
        coord_stats.min.x, coord_stats.min.y, coord_stats.min.z
    );
    println!(
        "  Max:    ({:.4}, {:.4}, {:.4}) nm",
        coord_stats.max.x, coord_stats.max.y, coord_stats.max.z
    );
    println!(
        "  Range:  ({:.4}, {:.4}, {:.4}) nm",
