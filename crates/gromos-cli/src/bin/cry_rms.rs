//! cry_rms - Crystallography RMSD analysis
//!
//! Usage: cry_rms @ref <file> @traj <file>
//!
...
fn main() {
    ...
    if args.len() < 5 {
        eprintln!("Usage: cry_rms @ref <file> @traj <file>");
        process::exit(1);
    }
    ...
    if ref_pos.is_empty() {
        eprintln!("Error: Could not load reference structure");
        process::exit(1);
    }
    ...
    loop {
        match traj.read_frame() {
            Ok(Some(frame)) => {
                let rmsd = calc_rmsd(&frame.positions, &ref_pos);
