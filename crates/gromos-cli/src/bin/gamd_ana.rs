...
fn parse_boost_file(path: &str) -> Result<Vec<GamdFrame>, String> {
    let file = File::open(path).map_err(|e| format!("Cannot open boost file {}: {}", path, e))?;
    ...
    for line in reader.lines() {
        ...
        let delta_v_total = parts[2]
            .parse()
            .map_err(|_| format!("Invalid ΔV: {}", parts[2]))?;
        let delta_v_dih = parts[3]
            .parse()
            .map_err(|_| format!("Invalid ΔV_dih: {}", parts[3]))?;
        let delta_v_pot = parts[4]
            .parse()
            .map_err(|_| format!("Invalid ΔV_tot: {}", parts[4]))?;
...
fn write_pmf(path: &str, frames: &[GamdFrame], free_energy_correction: f64) -> Result<(), String> {
    ...
    let file = File::create(path).map_err(|e| format!("Cannot create PMF file {}: {}", path, e))?;
    let mut writer = std::io::BufWriter::new(file);

    writeln!(writer, "# GaMD Reweighted Free Energy").map_err(|e| format!("Write error: {}", e))?;
    writeln!(
        writer,
        "# Free energy correction: {:.6} kJ/mol",
