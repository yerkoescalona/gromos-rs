...
use gromos::io::g96::G96Writer;
use gromos::io::pdb::PDBStructure;
use std::env;
use std::process;
...
fn main() {
    ...
    let output_g96 = if args.len() > 2 { Some(&args[2]) } else { None };
...
fn write_g96_from_pdb(path: &str, title: &str, pdb: &PDBStructure) -> Result<(), String> {
    ...
    use std::io::Write;
    writeln!(writer.writer.get_mut(), "POSITION").map_err(|e| format!("Write error: {}", e))?;
