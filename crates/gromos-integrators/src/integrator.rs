...
impl StochasticDynamics {
    ...
    fn compute_power_series(&self, dt: f64, _mass: f64, gamma_dt: f64) -> LangevinCoefficients {
        ...
        let exp_term =
            2.0 * gamma_dt - 2.0 * gamma_dt2 + 4.0 * gamma_dt3 / 3.0 - 2.0 * gamma_dt4 / 3.0;
        ...
        let term1 = dt
            * dt
            * (1.0 / 3.0 - gamma_dt / 4.0 + gamma_dt2 / 10.0 - gamma_dt3 / 36.0
                + gamma_dt4 / 144.0
                - gamma_dt5 / 600.0);
...
impl BerendsenThermostat {
    ...
    pub fn apply(&self, dt: f64, topo: &Topology, conf: &mut Configuration) {
        ...
        let lambda = (1.0
            + (dt / self.coupling_time) * (self.target_temperature / current_temp - 1.0))
            .sqrt();
...
mod tests {
    ...
    fn test_leap_frog_integrator() {
        ...
        // Initialize both current and old states since LeapFrog uses both
    ...
    fn test_energy_conservation() {
        ...
        // Calculate initial KE from old() to match where velocities will be after integration
        conf.old_mut().calculate_kinetic_energy(&topo.mass);
        let initial_ke = conf.old().energies.kinetic_total;
        ...
        // After steps, the updated velocities are in old() (due to internal exchange_state)
        conf.old_mut().calculate_kinetic_energy(&topo.mass);
        let final_ke = conf.old().energies.kinetic_total;

        // With no forces, kinetic energy should be conserved
