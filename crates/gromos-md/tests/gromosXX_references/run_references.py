...
SYSTEMS = [
    ...
    # --- Constraint algorithm references (SETTLE / LINCS / COM rotation) ---
    {
        "name": "nacl_1water_settle",
        "description": "Level 2: Na+ Cl- + 1 SPC water, SETTLE solvent constraints",
        "level": 2,
        "dir": "nacl_1water_settle",
        "topo": "nacl_1water_settle.topo",
        "conf": "nacl_1water_settle.conf",
        "input": "nacl_1water_settle.in",
        "pttopo": None,
        "natoms": 5,
        "isolates": "analytical SETTLE solver for rigid 3-site water (NTCS=settle)",
    },
    {
        "name": "nacl_1water_lincs",
        "description": "Level 2: Na+ Cl- + 1 SPC water, LINCS solvent constraints",
        "level": 2,
        "dir": "nacl_1water_lincs",
        "topo": "nacl_1water_lincs.topo",
        "conf": "nacl_1water_lincs.conf",
        "input": "nacl_1water_lincs.in",
        "pttopo": None,
        "natoms": 5,
        "isolates": "LINCS constraint solver applied to solvent (NTCS=lincs)",
    },
    {
        "name": "aladip_vacuum_lincs",
        "description": "Level 1: Alanine dipeptide in vacuum, LINCS solute constraints",
        "level": 1,
        "dir": "aladip_vacuum_lincs",
        "topo": "../../aladip.topo",
        "conf": "aladip_vacuum_lincs.conf",
        "input": "aladip_vacuum_lincs.in",
        "pttopo": None,
        "natoms": 12,
        "isolates": "LINCS constraint solver applied to solute bonds (NTC=3, NTCP=lincs)",
    },
    {
        "name": "water_216_box_com_rot",
        "description": "Level 3: 216 waters, NVE with COM translation+rotation removal",
        "level": 3,
        "dir": "water_216_box_com_rot",
        "topo": "../water_216_box/water_216_box.topo",
        "conf": "../water_216_box/water_216_box.conf",
        "input": "water_216_box_com_rot.in",
        "pttopo": None,
        "natoms": 648,
        "isolates": "COM rotation removal (NTICOM=2 initial, NSCM=-10 periodic)",
    },
]
...
def generate_toml(system, run_output, key_values, step0_forces):
    ...
    for key, val in key_values.items():
        ...
        toml_key = key.replace('(', '_').replace(')', '').replace(' ', '_').replace('-', '_')
    ...
    if step0_forces:
        ...
        lines.append(f'atom_0 = [{step0_forces[0][0]}, {step0_forces[0][1]}, {step0_forces[0][2]}]')
        if len(step0_forces) > 1:
            lines.append(f'atom_1 = [{step0_forces[1][0]}, {step0_forces[1][1]}, {step0_forces[1][2]}]')
        if len(step0_forces) > 2:
            lines.append(f'atom_2 = [{step0_forces[2][0]}, {step0_forces[2][1]}, {step0_forces[2][2]}]')
...
def main():
    ...
    if args.systems:
        ...
        if not systems_to_run:
            print(f"ERROR: No matching systems. Available: {[s['name'] for s in SYSTEMS]}")
            sys.exit(1)
    ...
    if failed == 0:
        ...
        print("Copy each system directory to gromos-rs/tests/data/ for use in Rust tests.")
    else:
