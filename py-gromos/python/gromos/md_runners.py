...
try:
    ...
    raise ImportError("gromos module not found. Please build with: maturin develop --release")
...
class MDSimulation:
    ...
    def __init__(
        self, topology_file: str, coordinate_file: str, input_file: str, output_prefix: str = "md"
    ):
        """
    ...
    def _find_binary(self, name: str) -> Path:
        ...
        release_path = (
            Path(__file__).parent.parent.parent / "gromos-rs" / "target" / "release" / name
        )
...
class GaMDSimulation(MDSimulation):
    ...
    def __init__(
        ...
        coordinate_file: str,
        input_file: str,
        output_prefix: str = "gamd",
        sigma0: float = 6.0,
        threshold_mode: str = "lower",
    ):
        """
    ...
    def run(
        self, equilibration_steps: int = 10000, production_steps: int = 100000, verbose: bool = True
    ) -> Dict[str, Path]:
        """
        Run GaMD simulation.
...
class REMDSimulation:
    ...
    def run(
        self, steps: int = 100000, exchange_interval: int = 1000, verbose: bool = True
    ) -> Dict[str, List[Path]]:
        """
        Run REMD simulation.
        ...
        trajectories = [
            Path(f"{self.output_prefix}_replica{i}.trc") for i in range(self.num_replicas)
        ]
        energies = [Path(f"{self.output_prefix}_replica{i}.tre") for i in range(self.num_replicas)]
...
def run_standard_md(
    topology: str, coordinates: str, input_file: str, steps: int = 10000, output_prefix: str = "md"
) -> Dict[str, Path]:
    """
...
def run_gamd(
    ...
    return sim.run(equilibration_steps=equilibration_steps, production_steps=production_steps)
...
class TISimulation:
    ...
    def run(
        self, steps_per_window: int = 100000, equilibration_steps: int = 10000, verbose: bool = True
    ) -> Dict[str, any]:
        """
        ...
        for i, lam in enumerate(self.lambda_values):
            ...
            cmd = [
                ...
                str(equilibration_steps),
                "--output",
                f"{self.output_prefix}_lambda{lam:.3f}",
            ]
