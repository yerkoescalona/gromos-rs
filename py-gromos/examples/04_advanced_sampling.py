...
eds_params = gromos.EDSParameters(num_states=num_states, smoothness=smoothness)
print(f"{eds_params}")
...
remd = gromos.ReplicaController(num_replicas=num_replicas, exchange_interval=exchange_interval)
print(f"{remd}")
...
print("  Exchange every {:.1f} ps (at dt=0.002 ps)".format(exchange_interval * 0.002))
