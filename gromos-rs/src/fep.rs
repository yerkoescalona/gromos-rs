...
mod tests {
    ...
    fn test_lambda_exponent() {
        let controller = LambdaController::new().with_lambda(0.5).with_exponent(2);
    ...
    fn test_bonded_lambda_derivative() {
        ...
        println!(
            "FEP Mini-test 1: dV/dλ at λ=0: {:.4}, λ=0.5: {:.4}, λ=1: {:.4}",
    ...
    fn test_softcore_prevents_singularity() {
        ...
        assert!(
            ...
            "Soft-core should increase distance at λ=0: {} vs {}",
            r_soft_lambda0,
            r_actual
        );
        ...
        assert!(
            r_soft_lambda05 < r_soft_lambda0,
            "Soft-core effect should decrease with λ"
        );

        // At λ = 1 (particle fully present), soft-core is inactive
        let r_soft_lambda1 = params.softcore_distance_lj(r_sq_close, 1.0, sigma);
        assert!(
            (r_soft_lambda1 - r_sq_close.sqrt()).abs() < 1e-6,
            "No soft-core at λ=1"
        );

        println!(
            "FEP Mini-test 2: Soft-core r_eff: λ=0: {:.4}, λ=0.5: {:.4}, λ=1: {:.4}",
    ...
    fn test_multi_window_consistency() {
        ...
        for (i, deriv) in derivatives.iter().enumerate() {
            assert!(
                deriv.is_finite(),
                "Derivative at window {} should be finite",
                i
            );
        }
    ...
    fn test_slow_growth_evolution() {
        ...
        // Check final lambda
        assert!(
            (controller.lambda - 1.0).abs() < 1e-6,
        ...
        for i in 0..lambda_history.len() - 1 {
            assert!(
                lambda_history[i + 1] >= lambda_history[i],
    ...
    fn test_softcore_electrostatics() {
        ...
        for lambda in &lambdas {
            ...
            if *lambda == 0.0 {
                assert!(
                    r_soft > 0.15,
                    "Electrostatic soft-core should be active at λ=0"
                );
            }
        ...
        for i in 0..r_soft_values.len() - 1 {
            assert!(
                r_soft_values[i] >= r_soft_values[i + 1],
