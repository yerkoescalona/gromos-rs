...
pub fn elapsed_time() -> f64 {
    unsafe { START_TIME.map_or(0.0, |start| start.elapsed().as_secs_f64()) }
}
...
impl EnergyDiagnostics {
    pub fn print(&self, verbose: bool) {
        if verbose {
            eprintln!("\n╔══════════════════════════════════════════════════════════════╗");
            eprintln!(
                "║  Energy Diagnostics - Step {:6}  Time {:8.3} ps        ║",
                self.step, self.time
            );
            eprintln!("╠══════════════════════════════════════════════════════════════╣");
            eprintln!(
                "║  Kinetic Energy:    {:12.4} kJ/mol                    ║",
                self.kinetic
            );
            eprintln!(
                "║  Potential Energy:  {:12.4} kJ/mol                    ║",
                self.potential
            );
            eprintln!(
                "║  Total Energy:      {:12.4} kJ/mol                    ║",
            ...
            eprintln!(
                "║  Dihedral:          {:12.4} kJ/mol                    ║",
                self.dihedral
            );
            eprintln!(
                "║  Improper:          {:12.4} kJ/mol                    ║",
                self.improper
            );
            eprintln!(
                "║  Lennard-Jones:     {:12.4} kJ/mol                    ║",
                self.lj
            );
            eprintln!(
                "║  Coulomb:           {:12.4} kJ/mol                    ║",
            ...
            log_info!(
                "Step {:6}  E_tot: {:10.4}  E_kin: {:10.4}  E_pot: {:10.4}  T: {:6.1} K",
                self.step,
                self.kinetic + self.potential,
                self.kinetic,
                self.potential,
...
impl ForceDiagnostics {
    pub fn print(&self) {
        ...
        log_debug!(
            "    Maximum force: {:.4} kJ/(mol·nm) on atom {}",
            self.max_force,
            self.max_force_atom + 1
        );
...
impl FrameDiagnostics {
    pub fn print(&self) {
        log_debug!(
            "Frame {}: time={:.3} ps, atoms={}, volume={:.3} nm³, COM=({:.3}, {:.3}, {:.3})",
            self.frame,
            self.time,
            self.num_atoms,
            self.box_volume,
