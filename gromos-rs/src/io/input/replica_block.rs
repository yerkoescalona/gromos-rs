        ...
        for line in reader.lines() {
            ...
            if in_replica_block {
                ...
                if !parsed_header {
                    if parts.len() < 3 {
                        return Err(format!(
                            "Invalid REPLICA header at line {}: expected NRET LRESCALE TEMPS",
                            line_num
                        ));
                    }

                    block.num_replicas = parts[0]
                        .parse::<usize>()
                        .map_err(|_| format!("Invalid NRET at line {}: {}", line_num, parts[0]))?;

                    let rescale = parts[1].parse::<i32>().map_err(|_| {
                        format!("Invalid LRESCALE at line {}: {}", line_num, parts[1])
                    })?;

                    block.rescale_velocities = rescale == 1;

                    // Parse temperature values (rest of the line)
                    block.temperatures = parts[2..]
                        .iter()
                        .map(|s| s.parse::<f64>())
                    ...
                    for (i, temp) in block.temperatures.iter().enumerate() {
                        if *temp <= 0.0 {
                            return Err(format!(
                                "Temperature {} must be positive, got {} at line {}",
                                i, temp, line_num
                            ));
                        }
                ...
                // Line 2: TRIALS  EQUILIBRATE
                    ...
                    if parts.len() != 2 {
                        return Err(format!(
                            "Invalid TRIALS line at {}: expected TRIALS EQUILIBRATE",
                            line_num
                        ));
                    }

                    block.exchange_interval = parts[0].parse::<usize>().map_err(|_| {
                        format!("Invalid TRIALS at line {}: {}", line_num, parts[0])
                    })?;

                    block.equilibration_steps = parts[1].parse::<usize>().map_err(|_| {
                        format!("Invalid EQUILIBRATE at line {}: {}", line_num, parts[1])
                    })?;

                    if block.exchange_interval == 0 {
                        return Err(format!(
                            "TRIALS must be greater than 0 at line {}",
                            line_num
                        ));
                    }
...
";
        ...
        assert!(result
            .unwrap_err()
            .contains("TRIALS must be greater than 0"));
    }
