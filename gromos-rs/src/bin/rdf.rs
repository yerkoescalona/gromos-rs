...
fn print_usage() {
    ...
    eprintln!("Usage:");
    eprintln!(
        "  rdf @topo <topology> @traj <trajectory> @group1 <atoms> @group2 <atoms> [@options]"
    );
    eprintln!();
    eprintln!("Arguments:");
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "@group1" | "@g1" => {
                i += 1;
                if i >= args.len() {
                    return Err("Missing value for @group1".to_string());
                }
            ...
            "@group2" | "@g2" => {
                i += 1;
                if i >= args.len() {
                    return Err("Missing value for @group2".to_string());
                }
            ...
            "@rmax" => {
                i += 1;
                if i >= args.len() {
                    return Err("Missing value for @rmax".to_string());
                }
                rdf_args.rmax = args[i]
                    .parse()
...
fn main() {
    ...
    let group1 = match AtomSelection::from_string(&rdf_args.group1_spec, &topo) {
        ...
        Err(e) => {
            eprintln!(
                "Error parsing group1 selection '{}': {}",
                rdf_args.group1_spec, e
            );
            process::exit(1);
        },
    };
    ...
    let group2 = match AtomSelection::from_string(&rdf_args.group2_spec, &topo) {
        ...
        Err(e) => {
            eprintln!(
                "Error parsing group2 selection '{}': {}",
                rdf_args.group2_spec, e
            );
            process::exit(1);
        },
    };
