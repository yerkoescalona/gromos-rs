...
fn print_usage() {
    ...
    eprintln!(
        "  @exchange_type  Exchange type: temperature, lambda, temp-lambda (default: temperature)"
    );
    eprintln!(
        "  @exchange_scheme Exchange scheme: sequential, odd-even, random (default: odd-even)"
    );
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "@cutoff" => {
                ...
                remd_args.cutoff = args[i].parse().map_err(|_| "Invalid cutoff".to_string())?;
            },
            "@tau_t" => {
                i += 1;
                if i >= args.len() {
                ...
                remd_args.tau_t = args[i].parse().map_err(|_| "Invalid tau_t".to_string())?;
            },
            "@shake" => {
                i += 1;
                if i >= args.len() {
...
fn main() {
    ...
    for (id, &temp) in args.temperatures.iter().enumerate() {
        ...
        let mut conf = Configuration::new(topology.num_atoms(), 1, 1);
    ...
    println!(
        "Exchange attempts: {}",
        args.n_steps / args.exchange_interval
    );
