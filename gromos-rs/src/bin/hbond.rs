...
use gromos_rs::{io::trajectory::TrajectoryReader, math::Vec3};
...
fn print_usage() {
    ...
    eprintln!();
    eprintln!(
        "Usage: hbond @traj <trajectory> [@hbdist <cutoff>] [@hbangle <angle>] [@output <file>]"
    );
    eprintln!();
    eprintln!("Arguments:");
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "@hbdist" => {
                i += 1;
                if i >= args.len() {
                    return Err("Missing value for @hbdist".to_string());
                }
                hb_args.distance_cutoff = args[i]
                    .parse()
            ...
            "@hbangle" => {
                i += 1;
                if i >= args.len() {
                    return Err("Missing value for @hbangle".to_string());
                }
                hb_args.angle_cutoff = args[i]
                    .parse()
...
fn main() {
    ...
    let mut output = File::create(&hb_args.output_file).expect("Failed to create output file");
    ...
    writeln!(
        output,
        "# {:>6} {:>6} {:>6} {:>8} {:>8} {:>8} {:>8}",
        "Donor", "H", "Accept", "Count", "Occupancy", "Avg_Dist", "Avg_Angle"
    )
    .unwrap();
