...
fn parse_replica_file(path: &str) -> Result<Vec<ExchangeEvent>, String> {
    let file = File::open(path).map_err(|e| format!("Cannot open replica file {}: {}", path, e))?;
    ...
    for line in reader.lines() {
        ...
        let step = parts[0]
            .parse()
            .map_err(|_| format!("Invalid step: {}", parts[0]))?;
        let time = parts[1]
            .parse()
            .map_err(|_| format!("Invalid time: {}", parts[1]))?;
        let replica_i = parts[2]
            .parse()
            .map_err(|_| format!("Invalid replica_i: {}", parts[2]))?;
        let replica_j = parts[3]
            .parse()
            .map_err(|_| format!("Invalid replica_j: {}", parts[3]))?;
        let temp_i = parts[4]
            .parse()
            .map_err(|_| format!("Invalid temp_i: {}", parts[4]))?;
        let temp_j = parts[5]
            .parse()
            .map_err(|_| format!("Invalid temp_j: {}", parts[5]))?;
        let probability = parts[6]
            .parse()
            .map_err(|_| format!("Invalid prob: {}", parts[6]))?;
        let accepted = parts[7]
            .parse::<i32>()
            .map_err(|_| format!("Invalid accepted: {}", parts[7]))?
            == 1;
...
fn write_statistics(
    ...
    writeln!(writer, "# Replica Exchange Statistics").map_err(|e| format!("Write error: {}", e))?;
    writeln!(writer, "#").map_err(|e| format!("Write error: {}", e))?;
    writeln!(writer, "# Overall Statistics:").map_err(|e| format!("Write error: {}", e))?;
