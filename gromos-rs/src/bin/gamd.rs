...
use gromos_rs::{
    ...
    gamd::{BoostForm, GamdParameters, GamdRunner, SearchMode, ThresholdType},
    integrator::LeapFrog,
...
fn read_coordinates(filename: &str) -> Result<Vec<Vec3>, String> {
    let file =
        File::open(filename).map_err(|e| format!("Could not open coordinate file: {}", e))?;
...
fn print_progress(step: usize, runner: &GamdRunner, config: &Configuration) {
...
fn main() -> Result<(), String> {
    ...
    if runner.gamd.search_mode != SearchMode::NoSearch && tot_stats.is_ready() {
        ...
        println!(
            "    gamd ... @mode production @k {:.6} @e {:.2}",
            runner.gamd.k_tot, runner.gamd.e_tot
        );
    }
