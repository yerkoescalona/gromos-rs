...
fn parse_vr_file(path: &str) -> Result<Vec<(usize, f64, f64)>, String> {
    let file = File::open(path).map_err(|e| format!("Cannot open V_R file {}: {}", path, e))?;
    ...
    for line in reader.lines() {
        ...
        let v_r = parts[2]
            .parse()
            .map_err(|_| format!("Invalid V_R: {}", parts[2]))?;
...
fn calculate_state_statistics(frames: &[EdsFrame]) -> Vec<StateStatistics> {
    ...
    for state_id in 0..num_states {
        let total_visits: usize = frames
            .iter()
            .filter_map(|f| f.visit_counts.get(state_id))
            .sum();

        let total_energy: f64 = frames
            .iter()
        ...
        let total_all_visits: usize = frames.iter().flat_map(|f| &f.visit_counts).sum();
...
fn write_free_energy_output(
    ...
    writeln!(writer, "# EDS Free Energy Analysis").map_err(|e| format!("Write error: {}", e))?;
