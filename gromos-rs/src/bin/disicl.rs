//! disicl - Distance between atoms in clusters
//!
//! Usage: disicl @topo <file> @traj <file> @atoms1 <i> @atoms2 <j>
//!
//! Calculates distances between two atom groups
...
fn main() {
    ...
    if args.len() < 9 {
        eprintln!("Usage: disicl @topo <file> @traj <file> @atoms1 <i> @atoms2 <j>");
        process::exit(1);
    }
        ...
        match traj.read_frame() {
            Ok(Some(frame)) => {
                if atom1 < frame.positions.len() && atom2 < frame.positions.len() {
                    ...
                    println!(
                        "{:12.4} {:12.6} {:12.6} {:12.6} {:12.6}",
                        frame.time, dist, dx, dy, dz
                    );
                }
            },
            Ok(None) => break,
