...
fn test_md_binary_simple_simulation() {
    ...
    assert!(
        Path::new(&traj_file).exists(),
        "Trajectory file not created"
    );
    ...
    assert!(
        traj_content.contains("TITLE"),
        "Trajectory missing TITLE block"
    );
    assert!(
        traj_content.contains("POSITIONRED"),
        "Trajectory missing POSITIONRED block"
    );
    assert!(
        traj_content.contains("GENBOX"),
        "Trajectory missing GENBOX block"
    );

    // Check energy file has content
    let ene_content = fs::read_to_string(&ene_file).expect("Failed to read energy file");
    assert!(
        ene_content.contains("TITLE"),
        "Energy file missing TITLE block"
    );
    assert!(
        ene_content.contains("ENERTRJ"),
        "Energy file missing ENERTRJ block"
    );

    // Count number of energy frames (should be at least 2 - initial and final)
    let energy_lines: Vec<&str> = ene_content
        .lines()
        .filter(|line| {
    ...
    assert!(
        energy_lines.len() >= 2,
        "Energy file should contain at least 2 frames, found: {}",
        energy_lines.len()
    );
    ...
    for (idx, line) in energy_lines.iter().take(2).enumerate() {
        ...
        assert!(
            values.len() >= 4,
            "Energy line {} should have at least 4 values, got: {}",
            idx,
            values.len()
        );
...
fn test_md_binary_energy_conservation() {
    ...
    assert!(
        energy_lines.len() >= 5,
        "Should have at least 5 energy frames"
    );
...
fn test_md_binary_help() {
    ...
    assert!(
        stderr.contains("Molecular Dynamics simulation")
            || stderr.contains("md - Molecular Dynamics"),
        "Help text should mention MD simulation"
    );
    assert!(
        stderr.contains("@topo"),
        "Help should mention @topo argument"
    );
    assert!(
        stderr.contains("@conf"),
        "Help should mention @conf argument"
    );
...
fn test_md_binary_missing_arguments() {
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("@topo") || stderr.contains("Usage"),
...
fn test_md_binary_fep_bond_perturbation() {
    ...
    assert!(
        energy_lines.len() >= 2,
        "Should have at least 2 energy frames"
    );
    ...
    for (idx, line) in energy_lines.iter().take(2).enumerate() {
        ...
        assert!(
            values.len() >= 4,
            "Energy line should have at least 4 values"
        );
...
fn test_md_binary_fep_lambda_scan() {
    ...
    for lambda in &lambda_values {
        ...
        if success {
            ...
            if Path::new(&ene_file).exists() {
                let ene_content =
                    fs::read_to_string(&ene_file).expect("Failed to read energy file");
                assert!(
                    ene_content.contains("ENERTRJ"),
                    "Energy file missing data at λ={}",
                    lambda
                );
            }
        }
    ...
    println!(
        "  - All {} lambda values simulated successfully",
        lambda_values.len()
    );
