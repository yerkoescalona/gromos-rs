...
fn test_cg16_single_point_energy() {
    // Load topology
    let parsed_topo = read_topology_file("tests/data/cg16.topo");
    ...
    // Load coordinates
    let mut conf = read_coordinate_file("tests/data/cg16.conf", 1, 1);
    ...
    for i in 0..topo.num_atoms().min(4) {
        println!(
            "Atom {}: IAC={}, mass={}, charge={}, pos=({:.3}, {:.3}, {:.3})",
            i,
            topo.iac[i],
            topo.mass[i],
            topo.charge[i],
    ...
    for i in 0..topo.num_atoms() {
        for j in (i + 1)..topo.num_atoms() {
            ...
            println!(
                "Pair {}-{}: dist={:.4}, C12={:.6e}, C6={:.6e}, E_LJ={:.6}",
...
fn test_load_both_files() {
    ...
    println!(
        "✓ Loaded {} atoms from topology and coordinates",
        topo.num_atoms()
    );
