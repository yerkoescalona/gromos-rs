...
use gromos_rs::configuration::{Box as SimBox, Configuration, State};
use gromos_rs::integrator::{Integrator, LeapFrog};
use gromos_rs::interaction::electrostatics::{
    pme_real_space_interaction, pme_self_energy, reaction_field_interaction, PMEParameters,
...
fn test_leapfrog_integrator() {
    ...
    assert!(
        (config.current().pos[0].x - 0.002).abs() < 1e-6,
        "Expected position ~0.002, got: {}",
        config.current().pos[0].x
    );

    // Velocity unchanged (no force)
    assert_eq!(
        config.current().vel[0].x,
        1.0,
