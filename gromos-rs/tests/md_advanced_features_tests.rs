...
use gromos_rs::integrator::{Integrator, LeapFrog};
use gromos_rs::interaction::nonbonded::{lj_crf_innerloop, CRFParameters, ForceStorage};
use gromos_rs::math::{Mat3, Rectangular, Vec3};
use gromos_rs::pairlist::{PairlistContainer, StandardPairlistAlgorithm};
use gromos_rs::topology::{Atom, Bond, BondParameters, LJParameters, Topology};
...
fn create_water_topology() -> Topology {
    ...
    let lj_o = LJParameters {
        c6: 0.0026,
        c12: 0.0025,
        cs6: 0.0026,
        cs12: 0.0025,
    };
    let lj_h = LJParameters {
        c6: 0.0,
        c12: 0.0,
        cs6: 0.0,
        cs12: 0.0,
    };
    let lj_oh = LJParameters {
        c6: 0.0,
        c12: 0.0,
        cs6: 0.0,
        cs12: 0.0,
    ...
    topo.lj_parameters = vec![vec![lj_o.clone(), lj_oh.clone()], vec![lj_oh, lj_h]];
...
fn test_nonbonded_lj_interaction() {
    ...
    assert!(
        storage.e_lj != 0.0,
        "LJ energy should be non-zero for interacting atoms"
    );

    // Forces should be opposite and equal (Newton's 3rd law)
    assert!(
        (storage.forces[0].x + storage.forces[1].x).abs() < 1e-5,
        "Forces should cancel (Newton's 3rd law)"
    );

    // Verify forces are non-zero
    assert!(
        storage.forces[0].length() > 0.0,
...
fn test_nonbonded_crf_interaction() {
    ...
    // Convert LJ parameters
    let lj_params: Vec<Vec<gromos_rs::interaction::nonbonded::LJParameters>> = topo
        .lj_parameters
        .iter()
        .map(|row| {
            row.iter()
                .map(|p| gromos_rs::interaction::nonbonded::LJParameters {
                    c6: p.c6,
                    c12: p.c12,
                })
                .collect()
...
fn test_berendsen_barostat_volume_control() {
    ...
    assert!(
        (final_volume - initial_volume).abs() > 1e-6,
...
fn test_shake_constraints_water() {
    ...
    assert!(
        result.iterations > 0,
        "SHAKE should perform at least one iteration"
    );
    assert!(
        result.iterations <= 1000,
        "SHAKE should not exceed max iterations"
    );
}
...
fn test_full_nvt_simulation() {
    ...
    assert!(
        conf.current().energies.kinetic_total > 0.0,
        "Should have kinetic energy"
    );

    // Should have LJ energy
    assert!(
        conf.current().energies.lj_total != 0.0,
        "Should have LJ energy"
    );
}
...
fn test_pairlist_update_frequency() {
    ...
    for step in 1..=10 {
        ...
        if step < 5 {
            assert!(
                !pairlist.needs_update(),
                "Should not need update before step 5"
            );
        } else if step == 5 {
            ...
            assert!(
                !pairlist.needs_update(),
                "Should not need update after reset"
            );
        } else {
            assert!(
                pairlist.needs_update(),
                "Should need update again at step 10"
            );
        }
    }
}
...
fn test_full_npt_simulation() {
    ...
    assert!(
        initial_volume > 0.0,
        "NPT simulation should complete successfully"
    );
}
...
fn test_full_nve_simulation() {
    ...
    for (i, &energy) in energies_history.iter().enumerate() {
        ...
        assert!(
            !energy.is_infinite(),
            "Energy at step {} should not be infinite",
            i
        );
    }
}
...
fn test_fep_simple_bond_perturbation() {
    ...
    use gromos_rs::interaction::bonded::{
        calculate_bond_forces_harmonic, calculate_perturbed_bond_forces,
    };
    ...
    for &lambda in &lambda_values {
        ...
        assert!(
            result.energy > 0.0,
            "Bond energy should be positive at λ={}",
            lambda
        );
        assert!(
            result.energy.is_finite(),
            "Energy should be finite at λ={}",
            lambda
        );
        assert!(
            result.lambda_derivative.is_finite(),
            "dE/dλ should be finite at λ={}",
            lambda
        );

        // Verify forces are non-zero (bond is stretched/compressed)
        assert!(
            result.forces[0].length() > 0.0,
            "Forces should be non-zero at λ={}",
            lambda
        );
        assert!(
            result.forces[1].length() > 0.0,
            "Forces should be non-zero at λ={}",
            lambda
        );
    ...
    assert!(
        (result_0.lambda_derivative - result_1.lambda_derivative).abs() > 0.01,
