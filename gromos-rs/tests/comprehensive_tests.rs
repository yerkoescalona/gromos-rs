...
use gromos_rs::math::{Periodicity, Rectangular, Vacuum, Vec3};
...
fn test_load_all_gromos_test_files() {
    let test_files = vec![(
        "tests/data/cg16.topo",
        "tests/data/cg16.conf",
    )];
...
fn test_topology_exclusions() {
    let topo_result = read_topology_file("tests/data/cg16.topo");
    ...
    for bond in &topo.solute.bonds {
        ...
        if bond.i < topo.exclusions.len() && bond.j < topo.exclusions.len() {
            assert!(
                topo.is_excluded(bond.i, bond.j),
                "Bonded atoms {}-{} should be excluded",
                bond.i,
                bond.j
            );
    ...
    for angle in &topo.solute.angles {
        println!(
            "Angle {}-{}-{}: checking 1-3 exclusion",
            angle.i, angle.j, angle.k
        );

        if angle.i < topo.exclusions.len() && angle.k < topo.exclusions.len() {
            assert!(
                topo.is_excluded(angle.i, angle.k),
                "1-3 pair {}-{} from angle should be excluded",
                angle.i,
                angle.k
            );
...
fn test_bonded_energies() {
    let topo_result = read_topology_file("tests/data/cg16.topo");
    let conf_result = read_coordinate_file("tests/data/cg16.conf", 1, 1);
    ...
    for bond in &topo.solute.bonds {
        ...
        println!(
            "Bond {}-{}: r={:.4} nm, r0={:.4} nm, E={:.6} kJ/mol",
    ...
    for angle in &topo.solute.angles {
        ...
        println!(
            "Angle {}-{}-{}: cos(θ)={:.4}, cos(θ0)={:.4}, E={:.6} kJ/mol",
            angle.i,
            angle.j,
            angle.k,
            cos_theta,
...
fn test_nonbonded_with_pbc() {
    ...
    if n_pairs > 0 {
        assert!(
            lj_energy < 0.0,
            "LJ energy should be negative for attractive interactions"
        );
    }
}
...
fn test_coordinate_parsing_precision() {
    ...
    for (i, vel) in conf.current().vel.iter().enumerate() {
        assert!(
            ...
            "Atom {} has NaN velocities",
            i
        );

        if i == 0 {
            println!(
                "First atom velocity: ({:.6}, {:.6}, {:.6})",
                vel.x, vel.y, vel.z
            );
        }
    }
...
fn test_lj_parameter_matrix() {
    ...
    println!(
        "LJ matrix size: {}x{}",
        topo.lj_parameters.len(),
        if topo.lj_parameters.is_empty() {
            0
        } else {
            topo.lj_parameters[0].len()
        }
    );
    ...
    for &iac in &topo.iac {
        if iac > 0 && iac <= topo.lj_parameters.len() {
            ...
            println!(
                "IAC {}: C6={:.6e}, C12={:.6e}, sigma={:.4} nm, epsilon={:.4} kJ/mol",
                iac,
                params.c6,
                params.c12,
                params.sigma(),
            ...
            if params.c6 > 0.0 || params.c12 > 0.0 {
                assert!(
                    params.sigma() > 0.0,
                    "Sigma should be positive for interacting atoms"
                );
...
fn test_full_energy_calculation() {
    ...
    println!("----------------------------------------");
    println!(
        "Total energy:    {:12.6} kJ/mol",
        bond_energy + angle_energy + lj_energy
    );
