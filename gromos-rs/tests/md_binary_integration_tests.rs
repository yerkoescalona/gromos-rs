...
fn parse_energy_file(path: &str) -> Result<Vec<f64>, String> {
    let content =
        fs::read_to_string(path).map_err(|e| format!("Cannot read energy file: {}", e))?;
...
fn test_md_binary_nve_simulation() {
    ...
    let result = run_md(&[
        ...
        "20",
        "@dt",
        "0.001", // Smaller timestep for stability
        "@traj",
        traj_file,
        "@ene",
        ene_file,
    ...
    assert!(
        check_output_file(traj_file).is_ok(),
        "Trajectory file not created or empty"
    );
    assert!(
        check_output_file(ene_file).is_ok(),
    ...
    assert!(
        !energy_values[0].is_nan(),
        "Initial energy should not be NaN"
    );
    assert!(
        !energy_values[0].is_infinite(),
        "Initial energy should not be infinite"
    );
...
fn test_md_binary_nvt_simulation() {
    ...
    assert!(
        !energies[0].is_nan() && !energies[0].is_infinite(),
...
fn test_md_binary_with_shake_constraints() {
    ...
    if !output.status.success() {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        panic!(
            "MD simulation with SHAKE failed:\nSTDOUT:\n{}\nSTDERR:\n{}",
            stdout, stderr
        );
    }
...
fn test_md_binary_short_vs_long_cutoff() {
    ...
    let result_short = run_md(&[
        "@topo", topo_file, "@conf", coord_file, "@steps", "10", "@dt", "0.002", "@traj",
        traj_short, "@ene", ene_short, "@cutoff", "0.8", // Short cutoff
    ]);
    ...
    let result_long = run_md(&[
        "@topo", topo_file, "@conf", coord_file, "@steps", "10", "@dt", "0.002", "@traj",
        traj_long, "@ene", ene_long, "@cutoff", "1.4", // Long cutoff
    ]);
    ...
    // Parse energies
    let energies_short =
        parse_energy_file(ene_short).expect("Failed to parse short cutoff energies");
...
fn test_md_binary_invalid_arguments() {
    ...
    let result = run_md(&[
        "@topo",
        "nonexistent_file.top",
        "@conf",
        "../test_tutorial/water.g96",
    ]);
    assert!(result.is_ok());
    let output = result.unwrap();
    assert!(
        !output.status.success(),
        "Should fail with nonexistent topology"
    );

    // Invalid parameter value
    let result = run_md(&[
        "@topo",
        "../test_tutorial/water_complete.top",
        "@conf",
        "../test_tutorial/water.g96",
        "@steps",
        "not_a_number",
    ...
    assert!(
        !output.status.success(),
        "Should fail with invalid @steps value"
    );
}
